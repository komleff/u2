...
2. Общая структура JSON-конфига корабля (Minimal)
...
6. hull — параметры корпуса (масса, прочность)
...
10. Nominals v0.8.6 Minimal — эталонные классы (для редактора)
...
11. Пример конфига Origin M50 Interceptor (v0.8.6 Minimal)
...
12. Миграция 0.6.4 → 0.8.6 Minimal (для ИИ-редактора)
...
13. Заглушки для будущего расширения (M7+)
