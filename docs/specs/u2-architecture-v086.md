...
- Температура - 0С (ноль градусов по Цельсию)
...
Автоматическое управление кораблем: плавное изменение курса и скорости. Используется для проверки работы симулятора в случаях, когда у игрока не работает управление (например, на мобильных устройствах без клавиатуры). Активируется автоматически по умолчанию в самом начале.
...
Не используется в текущей версии архитектуры и плана разработки. 

- Вероятность попадания - 0%;
- Урон - 0.

В будущем урон будет рассчитываться с учетом положения и скоростей кораблей (атакующего и цели) относительно друг друга и их ТТХ.
