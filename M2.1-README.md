...
### Differences from Specification Examples

The specification uses illustrative message names. Our implementation uses semantically equivalent but more precise naming:

| Specification | Implementation | Rationale |
|---------------|----------------|-----------|
| `ManeuverCommand` | `PlayerInputProto` | Clearer semantic meaning (player input vs. generic command) |
| `ManeuverCommand.tick` | `PlayerInputProto.sequence_number` | Better for lag compensation and input acknowledgment |
| `ManeuverCommand.entity_id` | `PlayerInputProto.client_id` | Server assigns entity in `ConnectionAcceptedProto` |

**Critical Addition**: The specification's `ManeuverCommand.flight_assist` field is implemented as `PlayerInputProto.flight_assist` (bool) to enable proper server-side physics replay during reconciliation (M2.2).

### Input-Snapshot Correlation Mechanism

For server reconciliation (M2.2+), the protocol uses:

```
Client → Server:
  PlayerInputProto { 
    sequence_number: N,  // Monotonically increasing
    timestamp_ms: T_client,
    control_state: { ... },
    flight_assist: true/false
  }

Server Processing:
  1. Receive input N at server tick T_server
  2. Store mapping: (sequence_number=N) → (tick=T_server)
  3. Apply input to entity and advance physics

Server → Client:
  WorldSnapshotProto {
    tick: T_server,      // Server tick when snapshot created
    timestamp_ms: T_snapshot,
    entities: [ ... ]
  }

Client Reconciliation:
  1. Receive snapshot for tick T_server
  2. Look up local predictions for inputs [N, N+1, ..., M]
  3. Compare local state at T_server with snapshot state
  4. Apply corrections if divergence detected
  5. Replay inputs [M+1, ...] with corrected state
```

**Key Design Decision**: Using `sequence_number` instead of `tick` in input messages allows:
- Client-side input buffering without knowing server tick rate
- Out-of-order packet detection
- Explicit input acknowledgment (server can send "last processed sequence")
- Flexibility in tick rate changes without protocol changes
